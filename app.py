...
# ==========================================
# 🛠️ 429 무료 API 과부하 방지용 함수들
# ==========================================
# 1. 이미지 크기 최적화 (토큰/전송량 획기적 절약)
def process_image(img):
    img_copy = img.copy()
    img_copy.thumbnail((800, 800), Image.Resampling.LANCZOS)
    return img_copy

# 2. 대기 시간 연장된 자동 재시도 함수
        ...
        try:
            ...
            if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
                if attempt < max_retries - 1:
                    wait_time = 20 * (attempt + 1)  # 20초, 40초, 60초로 대기 시간 대폭 증가
                    st.warning(f"⏳ 무료 API 일시적 과부하(429). 할당량 확보를 위해 {wait_time}초 대기 후 자동으로 재시도합니다... (시도 {attempt + 1}/{max_retries})")
...
if st.session_state['qna_mode']:
    ...
    user_question = st.text_area("질문 입력", height=150, placeholder="예: 생명과학 DNA 복제 과정이 너무 헷갈리는데, 쉽게 비유해서 설명해줄 수 있어?")
...
if st.session_state['material_mode']:
    ...
    if mat_img_file is not None:
        ...
        mat_img = process_image(raw_img) # 업로드 시 자동 용량 축소
        st.image(mat_img, caption="업로드된 사진 (최적화 완료)", use_container_width=True)
    
    mat_topic = st.text_area("1. 교재로 만들 핵심 주제나 원본 텍스트", height=120, placeholder="예: 고등학교 통합과학 - 빅뱅 우주론과 기본 입자의 생성")
    mat_request = st.text_area("2. 추가 요청사항 (선택 사항)", height=120, placeholder="예: 원소 생성 과정을 시간 순서대로 표로 정리하고, 이해를 돕기 위한 우주 배경 복사 사진을 본문에 넣어줘.")
        ...
        if api_key and (mat_topic.strip() or mat_img is not None):
            ...
            with st.spinner("자료를 분석하고 교재를 작성 중입니다..."):
                try:
                    ...
                    # 요청 양을 줄여 간결화된 프롬프트
                    prompt_complex = """
                    당신은 교재 제작 전문가입니다. 제공된 자료로 '고품질 복합 양식 학습 자료'를 만드세요.
                    [규칙]
                    1. 구글 검색을 통해 주제와 연관된 실제 인터넷 이미지 URL을 1~2개 찾아 `![설명](URL)` 형식으로 글 사이에 삽입하세요.
                    2. 특수문자 선 긋기(ASCII Art) 금지. 내용 시각화는 마크다운 표와 글머리 기호만 사용하세요.
                    """
...
if api_key:
    ...
    if mode == "일반 수행평가 준비":
        ...
        if uploaded_file is not None:
            if uploaded_file.name.lower().endswith('.txt'):
                ...
                ref_img = process_image(raw_img) # 업로드 시 자동 용량 축소
                st.success("사진 자료를 성공적으로 불러왔습니다.")
                st.image(ref_img, caption="업로드된 사진 (최적화 완료)", use_container_width=True)

        ref_text_input = st.text_area("1. 참고 텍스트", value=ref_text, height=150, placeholder="교과서 본문이나 참고할 지문을 붙여넣으세요.")
        guide_text = st.text_area("2. 수행평가 안내지 입력 (필수)", height=150, placeholder="예: '주제: 환경 오염의 해결 방안', 조건: 원인과 결과를 포함하여 500자 내외로 논술할 것.")
    ...
    # ------------------------------------------
        ...
        uploaded_img_file = st.file_uploader("암기할 사진 업로드 (.jpg, .png)", type=["jpg", "jpeg", "png"])
        uploaded_img = None
        if uploaded_img_file is not None:
            raw_img2 = Image.open(uploaded_img_file)
            uploaded_img = process_image(raw_img2) # 업로드 시 자동 용량 축소
        ...
        if st.button("📝 암기 테스트 문제 출제", use_container_width=True, type="primary"):
            if uploaded_img is not None or memo_text.strip():
                with st.spinner("암기 테스트를 만드는 중입니다..."):
                    try:
                        contents = []
                        if subject == "과학/수학 (공식 암기)": prompt_memo = "제공된 내용을 바탕으로 주관식 빈칸 3문제 출제. 정답 제외."
                        elif subject == "역사 (단어 및 개념 암기)": prompt_memo = "제공된 내용을 바탕으로 개념 확인 3문제 출제. 정답 제외."
                        elif subject == "영어 (지문 암기)" and eng_mode == "한글을 영어로 옮겨 적기 (영작)": prompt_memo = "영어 지문의 한글 해석을 제시하고 영작하는 문제 출제. 정답 제외."
                        elif subject == "영어 (지문 암기)" and eng_mode == "음성으로 말하기 테스트 (Speaking)": prompt_memo = "지문의 핵심 개요 힌트를 주고, '아래 마이크 버튼을 눌러 영어로 말해보세요' 안내 문구 출력."
                        else: prompt_memo = "제공된 내용에 대한 단답형 문제를 출제해 주세요. 정답 제외."
            ...
            if st.button("✨ AI 채점 및 예상 점수 받기"):
                if user_ans.strip():
                    with st.spinner("AI가 답안을 정밀 채점하고 있습니다..."):
                        ...
                        prompt_grade = f"""
                        [문제]: {st.session_state.get('practice_question', '')}
                        [내 답안]: {user_ans}
                        [조건]: {st.session_state.get('guide_text', '')}
                ...
                with st.spinner("채점 중입니다..."):
                    try:
                        ...
                        if is_speaking_mode:
                            ...
                            prompt_memo_grade = f"문제: {st.session_state['memo_test_q']}\n음성을 듣고 원본과 대조하여 간결히 채점해 주세요."
                        else:
                            prompt_memo_grade = f"문제: {st.session_state['memo_test_q']}\n답안: {user_memo_ans}\n원본 기준으로 간결히 채점해 주세요."
